...
#         def make(params, newparams , pumpsArray)
...
#             pumps = self.match( techParams, weightParams, additionalCriteria  , newparams , pumpsArray )
...
#         def match(techParams, weightParams, additionalCriteria , newparams , pumpsNamesArray)
#             matrix = PumpProperty::all
#             pumpsArray = Pump.where(name: pumpsNamesArray ).as_json
#             #build a hash of { pump.name => { property.name => pumpProperty.rating, .. }, ..}
#             pumpsHash = pumpsArray.map { |pump| [ pump['name'], {}] }.to_h            # #build a hash of { pump.name => { property.name => pumpProperty.rating, .. }, ..}
...
#                     if k.include? 'espcp' && pumpsHash['ESPCP']
#                         pumpsHash['ESPCP']['SE'] = [val.to_f, solution]
#                     elsif k.include? 'esp' && pumpsHash['ESP']
#                         pumpsHash['ESP']['SE'] = [val.to_f, solution]
#                     elsif k.include? 'pcp' && pumpsHash['PCP']
#                         pumpsHash['PCP']['SE'] = [val.to_f, solution]
#                     elsif k.include? 'rrp' && pumpsHash['RRP']
...
#                     xi = xi_pf
#                     solution = val.to_f * weightFactor / xi
#                     if k.include? 'espcp' && pumpsNamesArray.include? 'ESPCP' 
#                         pumpsHash['ESPCP']['PF'] = [val.to_f, solution]
#                     elsif k.include? 'esp' and pumpsNamesArray.include? 'ESP' 
