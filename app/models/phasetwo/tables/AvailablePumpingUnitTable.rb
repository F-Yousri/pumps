  class AvailablePumpingUnitTable 
    ...
    def prepare_input input
        @PT=(input[:PT])/1000
        @PPRL=input[:PPRL]/100
        @params={PT: @PT,PPRL: @PPRL,SL:input[:SL] }
    ...
    def get_data params
        ...
        @data = AvailablePumpingUnit.where("Max_GR > ?", params[:PT]).where("PPRL > ?", params[:PPRL]).where("max_stroke_length > ?", params[:SL]).last
        {PPRL100: @data['pprl'] ,PT1000:@data['max_gr'],SL:@data['max_stroke_length'] ,spuc:@data['cost']}
    end 
  end
